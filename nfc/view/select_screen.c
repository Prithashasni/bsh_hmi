...
static void scroll_event_cb(lv_event_t * e)
{
    ...
    uint32_t btn_count = lv_obj_get_child_cnt(cont) / 3; // original set count
    ...
    //         zoom = 280;
    //     }
    //     else if(diff_x < btn_w*1.2) {
    //         /* Near items = medium size */
    //         zoom = 230;
    //     }
    //     else if(diff_x < btn_w*2.2) {
    //         /* Far items = small size */
    //         zoom = 180;
    //     }
    //     else {
    //         /* Hide everything beyond 5 items */
    //         zoom = 120;
    //     }
    ...
    //     /* ---- OPACITY LOGIC ---- */
    //     lv_opa_t opa = LV_OPA_COVER - lv_map(diff_x, 0, 300, 0, LV_OPA_COVER - LV_OPA_30);
...
void create_select_screen()
{
    ...
    lv_obj_set_size(scr_select, 1024, 600);
    ...
    lv_obj_set_size(carousel_cont, 1024, 400);
    ...
    lv_obj_set_scroll_snap_x(carousel_cont, LV_SCROLL_SNAP_CENTER);
    ...
    lv_event_send(carousel_cont, LV_EVENT_SCROLL, NULL);
}
