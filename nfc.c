...
// Read NFC EEPROM and extract SSID/PASSWORD
int read_ssid_pw(char *out_ssid, char *out_pw){
    int fd = open(I2C_BUS, O_RDWR);
    if(fd < 0){ perror("[NFC] Open I2C failed"); return -1; }
    if(ioctl(fd, I2C_SLAVE, NFC_ADDR) < 0){ perror("[NFC] Set I2C addr failed"); close(fd); return -1; }

    uint8_t buf[READ_LEN];
    uint8_t start = 0x00;
    if(write(fd, &start, 1) != 1){ perror("[NFC] Set start address failed"); close(fd); return -1; }
    if(read(fd, buf, READ_LEN) != READ_LEN){ perror("[NFC] I2C read failed"); close(fd); return -1; }
    ...
    while(offset < READ_LEN - 4){
        ...
        if(len==0 || offset+4+len > READ_LEN){ offset++; continue; }
...
while(running){
    ...
    if(ret == 0){
        // Valid NFC read
        strncpy(cached_ssid, ssid, sizeof(cached_ssid)-1); cached_ssid[sizeof(cached_ssid)-1]=0;
        strncpy(cached_pw, pw, sizeof(cached_pw)-1); cached_pw[sizeof(cached_pw)-1]=0;
        wifi_update_credentials(ssid, pw, 0);
        printf("[NFC] Read credentials: SSID='%s', PW='%s'\n", ssid, pw);
    } else {
        // NFC failed → use cached
